...
class SLM_API_Listener
{
        ...
        if (isset($_REQUEST['slm_action']) && trim($_REQUEST['slm_action']) == 'slm_activate') {
            ...
            if ($retLic) {
                ...
                if (isset($_REQUEST['registered_domain']) && !empty($_REQUEST['registered_domain'])) {
                    $filtered_domains = array_filter($reg_domains, function ($domain) {
                        return strpos($domain->registered_domain, '_local') === false;
                    });
                    if (count($filtered_domains) < floor($retLic->max_allowed_domains) || strpos($_REQUEST['registered_domain'], '_local') !== false) {
                        ...
                        $message = ' Your license key: <strong>' . $key . '</strong> was activated successfully on ' . wp_date("F j, Y, g:i a") . '.<br><strong>Site: </strong>' .  $registered_domain;
    ...
    function deactivation_api_listener()
    {
        if (isset($_REQUEST['slm_action']) && trim($_REQUEST['slm_action']) == 'slm_deactivate') {
            ...
            $registered_domain  = isset($_REQUEST['registered_domain']) ? trim(wp_unslash(strip_tags($_REQUEST['registered_domain']))) : '';
            $license_key        = isset($_REQUEST['license_key']) ? trim(strip_tags($_REQUEST['license_key'])) : '';
            $registered_devices  = isset($_REQUEST['registered_devices']) ? trim(wp_unslash(strip_tags($_REQUEST['registered_devices']))) : '';
